...
def generate_bait_card(data=DATA, logo_path=LOGO_PATH, output=OUTPUT):
    ...
    return str(output.name) # Fixing POSTFIX json serialization error & returning only filename not entire path


def qr_bait(data=DATA, logo_path=LOGO_PATH, token=None): # CALLBACK_URL, ORG_LOGO, TOKEN
	output_path = DOWNLOADS_DIR / f"{token}.png" # How final qr is saved
	
    # print("OUTPUT: ", output_path, 'DATA: ', data, 'TOKEN: ', token)
    
	
	return generate_bait_card(data=DATA, logo_path=logo_path, output=output_path) # CALLBACK_URL, ORG_LOGO, DOWNLOAD_PATH
