...
TEMPLATE_DIR  = Path("static/baits/fim/")
TEMP_DIR 		  = TEMPLATE_DIR / "tmp"

DOWNLOADS_DIR = Path("static/downloads") # Where final Achieve is pushed
...
def fim_bait(callback_token: str, callback_url: str, template_dir: Path, monitored_path: str):
    deploy_injected = make_bait( callback_token=callback_token, callback_url=callback_url, monitored_path=monitored_path, template_dir=template_dir) #TEMPLATE_DIR / "deploy.ps1")
